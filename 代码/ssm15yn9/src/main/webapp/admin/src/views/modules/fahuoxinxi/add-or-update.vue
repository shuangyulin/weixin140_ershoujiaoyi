<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="商品状态" prop="shangpinzhuangtai">
          <el-select v-model="ruleForm.shangpinzhuangtai" placeholder="请选择商品状态">
            <el-option
                v-for="(item,index) in shangpinzhuangtaiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.kuaididanhao" label="快递单号" prop="kuaididanhao">
              <el-input v-model="ruleForm.kuaididanhao" 
                placeholder="快递单号" readonly></el-input>
          </el-form-item>
        </div>
  ...
  data() {
	...
	userid : false,
      ...
      ruleForm: {
        ...
        tupian: '',
        kuaidigongsi: '',
        kuaididanhao: this.getUUID(),
        userid: '',
      },
          shangpinzhuangtaiOptions: [],
      rules: {
          zhanghao: [
          ],
          yonghuxingming: [
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shangjiazhanghao!=''&&json.shangjiazhanghao){
          ...
          this.$message.error(data.msg);
            ...
            this.shangpinzhuangtaiOptions = "未发货,已发货,已签收".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `fahuoxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fahuoxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fahuoxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
